...
export function buildEmailVerificationTemplate(
  ...
  const subject = `${input.brandName} - Código de verificación`;

  const textLines = [
    `Hola ${input.recipientName},`,
    '',
    `Tu código de verificación para ${input.brandName} es: ${input.code}`,
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:14px;overflow:hidden;">
      <tr>
        <td style="padding:24px;background:linear-gradient(135deg,#1f6feb 0%,#0f9d7a 100%);color:#ffffff;">
          ...
          <h1 style="margin:10px 0 0;font-size:24px;line-height:1.2;">Verificación de correo</h1>
        </td>
      </tr>
      <tr>
        <td style="padding:24px;">
          <p style="margin:0 0 16px;font-size:16px;">Hola ${safeName},</p>
          <p style="margin:0 0 16px;font-size:15px;line-height:1.6;">Usa este código para terminar de verificar tu cuenta.</p>
          ...
          <p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#4b5563;">Este código expira en <strong>${input.expiresInMinutes} minutos</strong>.</p>
          ...
          ${safeSupportEmail ? `<p style="margin:0;font-size:13px;color:#6b7280;">Si no solicitaste este código, ignora este mensaje o escribe a <a href="mailto:${safeSupportEmail}" style="color:#1f6feb;">${safeSupportEmail}</a>.</p>` : ''}
        </td>
      </tr>
    </table>
  </body>
